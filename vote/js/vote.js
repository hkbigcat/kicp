...
function disableEditorFormat(){
    ...
    for (var i = 0; i < editorList.length; i++) {
        ...
        for(var j =0; j < editorOptions.length; j++){
            if (editorOptions[j].value != "full_html_vote") {
...
function resetVote(){
    if( !confirm('Are you sure that you want to RESET vote?') ){
...
function submitVote(){
    if( !confirm('Are you sure that you want to SUBMIT vote?') ){
        event.preventDefault();
    } else{
...
function EnterVote(name,post,dept){
    var msg ="The following information will be collected in this vote\n";
      if (name == 1) {msg=msg+"\n Voter's name";}
        if (post == 1) {msg=msg+"\n Post unit";}
        if (dept== 1) {msg=msg+"\n Department";}
